...
1. 定义 `DataSource` 接口，支持插件化数据源
2. 实现 `LocalDataSource` 作为默认实现
...
├── source.py           # DataSource Protocol + BaseDataSource
├── local_source.py     # LocalDataSource 实现
├── scanner.py          # DataSourceScanner 数据目录扫描
├── registry.py         # 数据源注册表
└── exceptions.py       # 自定义异常
```
...
- `src/caisen/data/source.py` ✓
- `src/caisen/data/local_source.py` ✓
- `src/caisen/data/scanner.py` ✓
