...
| 置信度 | 条件 | 说明 |
|--------|------|------|
| 0.95-1.0 | 完美形态 + 完美量能 + 强趋势 | 强烈买入 |
| 0.85-0.95 | 明确形态 + 良好量能 | 买入 |
| 0.70-0.85 | 形态形成 + 量能一般 | 轻仓买入 |
| < 0.70 | 形态模糊或量能不足 | 观望 |

### 五、具体信号规则

**买入（buy）**：同时满足趋势 + 形态 + 量能，置信度 ≥ 0.70

**卖出（sell）**：满足目标价/止损/趋势破坏任一条件
...
{
  "signals": [
    {"timestamp": "2024-01-10 11:00", "action": "sell", "confidence": 1.0, "reason": "止损触发：股价98<止损位98.5"}
  ]
}
```"""