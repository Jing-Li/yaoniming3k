"""蔡森形态 Prompt 测试脚本（简化版）

直接导入 prompt 模板，不依赖 caisen 包
"""
...
# 直接读取 prompt 文件
prompt_file = Path(__file__).parent / "prompts" / "caisen_pattern.py"
...
# 限制数据量以避免 token 过多（取最近 60 根）
sample_bars = bars[-60:] if len(bars) > 60 else bars
