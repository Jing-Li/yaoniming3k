...
| `/devtdd` | Source code (`.go`, `.java`, `.py`, etc.), test files, `internal/infra/http/openapi/` (OpenAPI declarations), `detail/api-contracts/openapi.yaml` (generated output) |
| `/arch-ops` | `ops/OPS.md`, `scripts/` content (incl. `gen-openapi.sh`), `Makefile` (incl. `openapi` target) |
...
| `/arch-review` | At least one upstream skill T{N} = done (full audit requires devtdd done) |
...
**Exception**: Migration Mode bypasses the upstream halt when all blueprints were just generated by the migration pipeline. See [kanban-spec.md §10 Migration Mode Protocol](kanban-spec.md#10-migration-mode-protocol-v210) for the full protocol, or each skill's Migration Mode Detection section.
